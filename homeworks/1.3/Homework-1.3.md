...
###### Breakdown of `hw1-3.py`

- The program defines a route for `hw1` using a python decorator
- It the connect to the database `m101` and accesses the collection `funnynumbers`
- It then filters the collection by `limit=1, skip=n` &rarr; [db.collection.find()](https://docs.mongodb.com/manual/reference/method/db.collection.find/)
...
- The `n` is a user supplied parameter via the URL
- The program will sort the records according to the values and skip the first `n` records.
...
- Assuming the value of `n` is `50`

![](images/14.png)

![](images/15.png)

- Assuming the value of `n` is `57`
