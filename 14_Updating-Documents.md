...
$pullAll|Removes all matching values from an array.
 ...
 like. 
 - Other content such as reviews and ratings will need to be updated as users take action. 
 - We could use `$set` for this purpose, but that's an error prone approach. 
 - It's too easy to do the arithmetic incorrectly. 
 - Instead, we have a number of operators that support numeric updates of data: `$min, $max, $inc, $mul`.
